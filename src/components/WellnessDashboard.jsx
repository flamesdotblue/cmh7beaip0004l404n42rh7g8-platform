...
export default function WellnessDashboard({ persistMode }) {
    ...
    <div className="rounded-2xl bg-white/5 border border-white/10 backdrop-blur-md p-4 md:p-6">
      <div className="grid md:grid-cols-3 gap-6">
        <div className="md:col-span-2">
          <h4 className="text-white/80 text-sm mb-2 flex items-center gap-2"><Activity size={18}/> Mood tracker</h4>
          <div className="rounded-lg border border-white/15 bg-white/10 p-3">
            <div className="flex items-center gap-3">
              <input type="range" min={1} max={10} value={mood} onChange={(e)=>setMood(Number(e.target.value))} />
              <span className="text-sm text-white/80">{mood}/10 • {scaleLabel(mood)}</span>
            </div>
            <div className="mt-3 flex gap-2">
              <input value={note} onChange={(e)=>setNote(e.target.value)} placeholder="Optional note" className="flex-1 h-10 rounded bg-white/10 border border-white/15 px-3 outline-none placeholder:text-white/40" />
              <button onClick={addMood} className="h-10 px-4 rounded-lg bg-gradient-to-r from-violet-500 to-fuchsia-500 hover:opacity-95 transition">Add</button>
            </div>
            <div className="mt-4 grid grid-cols-5 gap-3">
              {moodCounts.map(m => (
                <div key={m.key} className="text-center">
                  ...
                  <div className="mt-1 text-xs text-white/70 capitalize">{m.key}</div>
                  <div className="text-xs text-white/60">{m.count}</div>
                </div>
              ))}
            </div>
          </div>

          <h4 className="text-white/80 text-sm mt-6 mb-2 flex items-center gap-2"><Calendar size={18}/> Recent mood notes</h4>
          <div className="grid md:grid-cols-2 gap-3 max-h-64 overflow-y-auto pr-1">
            {moodHistory.slice().reverse().slice(0,12).map((e, i) => (
              <div key={i} className="rounded-lg border border-white/15 bg-white/5 p-3">
                ...
                {e.sample && <p className="mt-2 text-sm text-white/90">{e.sample}</p>}
              </div>
            ))}
          </div>
        </div>
        ...
        <div>
          <h4 className="text-white/80 text-sm mb-2 flex items-center gap-2"><Trophy size={18}/> Daily challenges</h4>
          <div className="rounded-lg border border-white/15 bg-white/10 p-3 space-y-2">
            {DAILY_CHALLENGES.map(ch => (
              <label key={ch.id} className="flex items-center justify-between gap-3 p-2 rounded bg-white/5 border border-white/10 hover:bg-white/10 cursor-pointer">
                <div className="text-sm text-white/90">{ch.label}</div>
                <input type="checkbox" className="accent-violet-500 h-4 w-4" checked={!!progress.done[ch.id]} onChange={()=>toggleChallenge(ch.id)} />
              </label>
            ))}
            <div className="text-xs text-white/70">Points today: {points}</div>
          </div>

          <h4 className="text-white/80 text-sm mt-6 mb-2 flex items-center gap-2"><Award size={18}/> Achievements</h4>
          <div className="rounded-lg border border-white/15 bg-white/10 p-3 space-y-2">
            {badges.length === 0 && <div className="text-xs text-white/60">Complete challenges and track moods to earn badges.</div>}
            {badges.map((b, i) => (
              <div key={i} className="flex items-center gap-2 text-sm text-white/90"><CheckCircle size={16} className="text-emerald-400"/> {b}</div>
            ))}
            <div className="text-xs text-white/60 mt-2">Streak: {streak} days</div>
          </div>
        </div>
      </div>
    </div>
